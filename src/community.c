...
int igraph_i_community_eb_get_merges2(const igraph_t *graph,
    ...
    if (modularity || res || bridges) {
        ...
        if (modularity) {
            IGRAPH_CHECK(
                igraph_vector_resize(modularity, no_of_nodes - no_comps + 1));
        }
        if (res) {
    ...
    for (i = igraph_vector_size(edges) - 1; i >= 0; i--) {
        ...
        if (c1 != c2) { /* this is a merge */
            ...
            IGRAPH_CHECK(
                igraph_modularity(graph, &mymembership, &actmod, weights));
            if (modularity) {
...
int igraph_community_eb_get_merges(const igraph_t *graph,
    ...
    if (membership || modularity) {
        return igraph_i_community_eb_get_merges2(
            graph, edges, weights, res, bridges, modularity, membership);
    }
...
int igraph_community_edge_betweenness(
    ...
    if (result == 0) {
        ...
        if (result == 0)
            IGRAPH_ERROR("edge betweenness community structure failed",
                         IGRAPH_ENOMEM);
    ...
    if (weights == 0) {
        ...
        IGRAPH_CHECK(
            igraph_inclist_init_empty(&fathers, (igraph_integer_t)no_of_nodes));
    ...
    for (e = 0; e < no_of_edges; steps_done += no_of_edges - e, e++) {
        ...
        if (weights == 0) {
            ...
            for (source = 0; source < no_of_nodes; source++) {
                ...
                memset(nrgeo, 0,
                       (size_t)no_of_nodes * sizeof(unsigned long long int));
                ...
                while (!igraph_dqueue_empty(&q)) {
                    ...
                    for (i = 0; i < neino; i++) {
                        igraph_integer_t edge =
                                             (igraph_integer_t)VECTOR(*neip)[i],
                                         from, to;
                        long int neighbor;
                        igraph_edge(graph, edge, &from, &to);
                        neighbor = actnode != from ? from : to;
                        if (nrgeo[neighbor] != 0) {
                            /* we've already seen this node, another shortest
                             * path? */
            ...
            for (source = 0; source < no_of_nodes; source++) {
                ...
                while (!igraph_2wheap_empty(&heap)) {
                    ...
                    for (i = 0; i < neino; i++) {
                        ...
                        if (curdist == 0) {
                            ...
                            IGRAPH_CHECK(igraph_2wheap_push_with_index(
                                &heap, to, -altdist));
                ...
                while (!igraph_stack_empty(&stack)) {
                    ...
                    for (i = 0; i < fatv_len; i++) {
                        ...
                        tmpscore[neighbor] +=
                            (tmpscore[w] + 1) * nrgeo[neighbor] / nrgeo[w];
                        VECTOR(eb)
                        [fedge] +=
                            (tmpscore[w] + 1) * nrgeo[neighbor] / nrgeo[w];
                    }
    ...
    if (merges || bridges || modularity || membership) {
        IGRAPH_CHECK(igraph_community_eb_get_merges(
            graph, result, weights, merges, bridges, modularity, membership));
    }
...
int igraph_community_to_membership(const igraph_matrix_t *merges,
    ...
    if (steps > igraph_matrix_nrow(merges)) {
        IGRAPH_ERROR("`steps' to big or `merges' matrix too short",
                     IGRAPH_EINVAL);
    }
...
int igraph_modularity(const igraph_t *graph, const igraph_vector_t *membership,
    ...
    if (igraph_is_directed(graph)) {
        IGRAPH_ERROR("modularity is implemented for undirected graphs",
                     IGRAPH_EINVAL);
    }
 ...
 * vector.
 *
 * This function was originally contributed by Tom Gregorovic, and was
 * later improved by Li-Heng Liou.
 *
 * \param  membership  Numeric vector which gives the type of each
 *                     vertex, ie. the component to which it belongs.
...
int igraph_reindex_membership(igraph_vector_t *membership,
    ...
    long int max_cid = igraph_vector_max(membership);
    igraph_vector_t record;
    igraph_vector_t *new_to_old_real;
    ...
    /* Create community id record */
    IGRAPH_VECTOR_INIT_FINALLY(&record, max_cid+1);
    igraph_vector_fill(&record, -1);
    long int last_id=0;
    for(long int i =0; i<size; i++){
        long int this_id=VECTOR(*membership)[i];
        if(VECTOR(record)[this_id]==-1){
            VECTOR(record)[this_id]=last_id++;
            IGRAPH_CHECK(igraph_vector_push_back(new_to_old_real, this_id));
        }
        VECTOR(*membership)[i]=VECTOR(record)[this_id];
    }
    igraph_vector_destroy(&record);
    IGRAPH_FINALLY_CLEAN(1);
...
void igraph_i_error_handler_none(const char *reason, const char *file, int line,
                                 int igraph_errno) {
...
int igraph_community_leading_eigenvector(
    ...
    if (start && membership && igraph_vector_size(membership) != no_of_nodes) {
    ...
    if (!start) {
        /* Calculate the weakly connected components in the graph and use them
         * as
         * an initial split */
        IGRAPH_CHECK(
            igraph_clusters(graph, mymembership, &idx, 0, IGRAPH_WEAK));
        communities = igraph_vector_size(&idx);
        IGRAPH_STATUSF(("Starting from %li component(s).\n", 0, communities));
        if (history) {
            IGRAPH_CHECK(
                igraph_vector_push_back(history, IGRAPH_LEVC_HIST_START_FULL));
        }
    } else {
        ...
        if (history) {
            IGRAPH_CHECK(
                igraph_vector_push_back(history, IGRAPH_LEVC_HIST_START_GIVEN));
    ...
    for (i = 1; i < communities; i++) {
        ...
        if (eigenvectors) {
            ...
            if (!v) {
                IGRAPH_ERROR(
                    "Cannot do leading eigenvector community detection",
                    IGRAPH_ENOMEM);
            }
        ...
        if (history) {
            IGRAPH_CHECK(
                igraph_vector_push_back(history, IGRAPH_LEVC_HIST_SPLIT));
    ...
    while (!igraph_dqueue_empty(&tosplit) && staken < steps) {
        ...
        if (callback) {
            ...
            ret = callback(mymembership, comm, storage.d[0], &vv, arpcb1,
        ...
        if (storage.d[0] <= 0) {
            ...
            if (history) {
                IGRAPH_CHECK(
                    igraph_vector_push_back(history, IGRAPH_LEVC_HIST_FAILED));
    ...
    if (modularity) {
        IGRAPH_CHECK(
            igraph_modularity(graph, mymembership, modularity, weights));
    }
 ...
 *     on output the resulting membership vector, after performing \c steps
 * merges.
...
int igraph_le_community_to_membership(const igraph_matrix_t *merges,
    ...
    if (components > no_of_nodes) {
        IGRAPH_ERROR("Invalid membership vector, too many components",
                     IGRAPH_EINVAL);
    }
    if (steps >= components) {
        IGRAPH_ERROR(
            "Cannot make `steps' steps from supplied membership vector",
            IGRAPH_EINVAL);
    }
    ...
    for (i = 0; i < no_of_nodes; i++) {
        if (VECTOR(*membership)[i] < 0) {
            IGRAPH_ERROR("Invalid membership vector, negative id",
                         IGRAPH_EINVAL);
        }
    ...
    for (i = 0; i < components; i++) {
        if (VECTOR(fake_memb)[i] == 0) {
            IGRAPH_ERROR("Invalid membership vector, empty cluster",
                         IGRAPH_EINVAL);
        }
    }

    IGRAPH_CHECK(igraph_community_to_membership(
        merges, (igraph_integer_t)components, steps, &fake_memb, 0));
    ...
    for (i = 0; i < no_of_nodes; i++) {
        VECTOR(*membership)
        [i] = VECTOR(fake_memb)[(long int)VECTOR(*membership)[i]];
        if (csize) {
...
int igraph_community_label_propagation(const igraph_t *graph,
    ...
    if (initial) {
        if (igraph_vector_size(initial) != no_of_nodes) {
            IGRAPH_ERROR("Invalid initial labeling vector length",
                         IGRAPH_EINVAL);
        }
        ...
        if (i > no_of_nodes) {
            IGRAPH_ERROR("elements of the initial labeling vector must be "
                         "between 0 and |V|-1",
                         IGRAPH_EINVAL);
        }
        if (i <= 0) {
            IGRAPH_ERROR(
                "at least one vertex must be labeled in the initial labeling",
                IGRAPH_EINVAL);
        }
    } else {
        for (i = 0; i < no_of_nodes; i++) {
        ...
        for (i = 0; i < no_of_not_fixed_nodes; i++) {
            ...
            if (weights) {
                ...
                for (j = 0; j < num_neis; j++) {
                    k = (long int)VECTOR(*membership)[(long)IGRAPH_OTHER(
                        graph, VECTOR(*ineis)[j], v1)];
                    if (k == 0)
                        continue; /* skip if it has no label yet */
                    was_zero = (VECTOR(label_counters)[k] == 0);
                    VECTOR(label_counters)
                    [k] += VECTOR(*weights)[(long)VECTOR(*ineis)[j]];
                    if (was_zero && VECTOR(label_counters)[k] != 0) {
                        /* counter just became nonzero */
                        IGRAPH_CHECK(
                            igraph_vector_push_back(&nonzero_labels, k));
                    }
            ...
            if (igraph_vector_size(&dominant_labels) > 0) {
                ...
                if (VECTOR(label_counters)[(long)VECTOR(*membership)[v1]] !=
                    max_count) {
    ...
    if (modularity) {
        IGRAPH_CHECK(igraph_modularity(graph, membership, modularity, weights));
    }
...
typedef struct {
    long int communities_no,
        vertices_no;          /* Number of communities, number of vertices */
...
igraph_real_t igraph_i_multilevel_community_modularity(
    ...
    for (i = 0; i < communities->vertices_no; i++) {
        if (communities->item[i].size > 0) {
            result += (communities->item[i].weight_inside -
                       communities->item[i].weight_all *
                           communities->item[i].weight_all / m) /
                      m;
        }
    }
...
/* removes multiple edges and returns new edge id's for each edge in O(|E|)
 */
int igraph_i_multilevel_simplify_multiple(igraph_t *graph,
    ...
    IGRAPH_VECTOR_INIT_FINALLY(&record, vcount);
    igraph_vector_fill(&record, -1);
    IGRAPH_VECTOR_INIT_FINALLY(&new_eids, vcount);
    IGRAPH_VECTOR_INIT_FINALLY(&edge, 0);

    for (long int i = 0; i < vcount; i++) {
        igraph_vector_t incident_edges;
        IGRAPH_VECTOR_INIT_FINALLY(&incident_edges, 0);
        igraph_incident(graph, &incident_edges, i, IGRAPH_OUT);
        long int n = igraph_vector_size(&incident_edges);
        for (long int j = 0; j < n; j++) {
            long int eidx = (long int)VECTOR(incident_edges)[j];
            long int to = IGRAPH_OTHER(graph, eidx, i);
            if (VECTOR(record)[to] != i) {
                VECTOR(record)[to] = i;
                igraph_vector_push_back(&edge, i);
                igraph_vector_push_back(&edge, to);
                VECTOR(new_eids)[to] = necount++;
            }
            VECTOR(*eids)[eidx] = VECTOR(new_eids)[to];
        }
    ...
    IGRAPH_CHECK(igraph_create(graph, &edge, igraph_vcount(graph), directed));
 ...
 */
int igraph_i_multilevel_community_links(
    const igraph_t *graph,
    const igraph_i_multilevel_community_list *communities,
    igraph_integer_t vertex, igraph_vector_t *edges, igraph_real_t *weight_all,
    igraph_real_t *weight_inside, igraph_real_t *weight_loop,
    igraph_vector_t *links_community, igraph_vector_t *links_weight,
    igraph_i_multilevel_community_link *links, igraph_vector_t *record) {

    long int i, n, last = -1, c = 0;
    igraph_real_t weight = 1;
    long int to, to_community;
    long int community =
        (long int)VECTOR(*(communities->membership))[(long int)vertex];
        ...
        *weight_all += weight;
        if (to == vertex) {
            *weight_loop += weight;
            ...
            if (VECTOR(*record)[community] != vertex) {
                VECTOR(*record)[community] = vertex;
                links[community].weight = 0;
                links[community].prev_cid = last;
                last = community;
                c++;
            }
            continue;
        ...
        if (VECTOR(*record)[to_community] != vertex) {
            VECTOR(*record)[to_community] = vertex;
            links[to_community].weight = 0;
            links[to_community].prev_cid = last;
            last = to_community;
            c++;
        }
        links[to_community].weight += weight;
    }

    igraph_vector_reserve(links_community, c);
    igraph_vector_reserve(links_weight, c);

    while (last != -1) {
        igraph_vector_push_back(links_community, last);
        igraph_vector_push_back(links_weight, links[last].weight);
        last = links[last].prev_cid;
    }
...
igraph_real_t igraph_i_multilevel_community_modularity_gain(
    ...
    return weight_inside - communities->item[(long int)community].weight_all *
                               weight_all / communities->weight_sum;
}
 ...
 * \brief Performs a single step of the multi-level modularity optimization
 * method
 *
 * This function implements a single step of the multi-level modularity
 * optimization
 ...
 *
 * This function was originally contributed by Tom Gregorovic, and was later
 * improved by Li-Heng Liou.
 *
...
int igraph_i_community_multilevel_step(igraph_t *graph,
    ...
    if (igraph_vector_size(weights) < igraph_ecount(graph))
        IGRAPH_ERROR("multi-level community detection: weight vector too short",
                     IGRAPH_EINVAL);
    ...
    if (communities.item == 0) {
        IGRAPH_ERROR("multi-level community structure detection failed",
                     IGRAPH_ENOMEM);
    }
    IGRAPH_FINALLY(igraph_free, communities.item);
    links = igraph_Calloc(vcount, igraph_i_multilevel_community_link);
    if (links == 0) {
        IGRAPH_ERROR("multi-level community structure detection failed",
                     IGRAPH_ENOMEM);
    }
    ...
    do { /* Pass begin */
        ...
        /* Save the current membership, it will be restored in case of worse
         * result */
        IGRAPH_CHECK(
            igraph_vector_update(&temp_membership, communities.membership));
        ...
        for (i = 0; i < vcount; i++) {
            ...
            igraph_i_multilevel_community_links(
                graph, &communities, (igraph_integer_t)i, &edges, &weight_all,
                &weight_inside, &weight_loop, &links_community, &links_weight,
                links, &record);
            ...
            communities.item[old_id].weight_inside -=
                2 * weight_inside + weight_loop;
            ...
            for (j = 0; j < n; j++) {
                ...
                igraph_real_t q_gain =
                    igraph_i_multilevel_community_modularity_gain(
                        &communities, (igraph_integer_t)c, (igraph_integer_t)i,
                        weight_all, w);
                /* debug("Link %ld -> %ld weight: %lf gain: %lf\n", i, c,
                 * (double) w, (double) q_gain); */
            ...
            communities.item[new_id].weight_inside +=
                2 * max_weight + weight_loop;
        ...
        if (changed && (q > pass_q)) {
            /* debug("Pass %d (changed: %d) Communities: %ld Modularity from %lf
              to %lf\n",
              pass, changed, communities.communities_no, (double) pass_q,
              (double) q); */
            pass++;
        } else {
            /* No changes or the modularity became worse, restore last
             * membership */
            IGRAPH_CHECK(
                igraph_vector_update(communities.membership, &temp_membership));
    ...
    /* Shrink the nodes of the graph according to the present community
     * structure
    ...
    /* Here we reuse the edges vector as we don't need the previous contents
     * anymore */
    ...
    while (1) {
        ...
        if (memberships || membership) {
            for (i = 0; i < vcount; i++) {
                ...
                VECTOR(level_membership)
                [i] = VECTOR(m)[(long int)VECTOR(level_membership)[i]];
            }
        }

        if (modularity) {
            /* If we have to return the modularity scores, add it to the
             * modularity vector */
            IGRAPH_CHECK(igraph_vector_push_back(modularity, q));
        }

        if (memberships) {
            /* If we have to return the membership vectors at each level, store
             * the new
             * membership vector */
            IGRAPH_CHECK(igraph_matrix_add_rows(memberships, 1));
            IGRAPH_CHECK(igraph_matrix_set_row(memberships, &level_membership,
                                               level - 1));
        }
...
int igraph_compare_communities(const igraph_vector_t *comm1,
    ...
    if (igraph_vector_size(comm1) != igraph_vector_size(comm2)) {
        IGRAPH_ERROR("community membership vectors have different lengths",
                     IGRAPH_EINVAL);
    }
    ...
    case IGRAPH_COMMCMP_ADJUSTED_RAND:
        IGRAPH_CHECK(igraph_i_compare_communities_rand(
            &c1, &c2, result, method == IGRAPH_COMMCMP_ADJUSTED_RAND));
        break;
 ...
 * \param  comm1       the membership vector of the first community structure
 * \param  comm2       the membership vector of the second community structure
 * \param  distance12  pointer to an \c igraph_integer_t, the projection
 * distance
 *                     of the first community structure from the second one will
 * be
 *                     returned here.
 * \param  distance21  pointer to an \c igraph_integer_t, the projection
 * distance
 *                     of the second community structure from the first one will
 * be
 *                     returned here.
 * \return  Error code.
 *
...
int igraph_split_join_distance(const igraph_vector_t *comm1,
    ...
    IGRAPH_CHECK(
        igraph_i_split_join_distance(&c1, &c2, distance12, distance21));
...
int igraph_i_entropy_and_mutual_information(const igraph_vector_t *v1,
    ...
    if (p1 == 0) {
        IGRAPH_ERROR("igraph_i_entropy_and_mutual_information failed",
                     IGRAPH_ENOMEM);
    }
    ...
    *mut_inf = 0.0;
    ...
    for (i = 0; i < n; i++) {
        IGRAPH_CHECK(igraph_spmatrix_add_e(&m, (int)VECTOR(*v1)[i],
                                           (int)VECTOR(*v2)[i], 1));
    }
...
int igraph_i_compare_communities_nmi(const igraph_vector_t *v1,
    ...
    IGRAPH_CHECK(
        igraph_i_entropy_and_mutual_information(v1, v2, &h1, &h2, &mut_inf));
 ...
 */
int igraph_i_confusion_matrix(const igraph_vector_t *v1,
                              const igraph_vector_t *v2, igraph_spmatrix_t *m) {
    long int k1 = (long int)igraph_vector_max(v1) + 1;
    long int k2 = (long int)igraph_vector_max(v2) + 1;
    long int i, n = igraph_vector_size(v1);
...
int igraph_i_compare_communities_rand(const igraph_vector_t *v1,
    ...
    for (i = 0; i < nrow; i++) {
        frac_pairs_in_1 +=
            (VECTOR(rowsums)[i] / n) * (VECTOR(rowsums)[i] - 1) / (n - 1);
    }
    for (i = 0; i < ncol; i++) {
        frac_pairs_in_2 +=
            (VECTOR(colsums)[i] / n) * (VECTOR(colsums)[i] - 1) / (n - 1);
    }
